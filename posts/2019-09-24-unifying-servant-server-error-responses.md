...
- B. ServantError type Exceptions thrown within API route handlers by a user.
- C. IO asynchronous and impure synchronous Exceptions either thrown by non-total functions or external IO failures such as DB failures.


## Servant server error handling

Errors of `category A and B` have servant framework handling support, while Errors of `category C` seep through the Servant framework layer to an underlying server such as the warp-wai server.
...
- B. There is no way we can customise the response body to alter its form.
...
In the event that a user submits a wrong request body during an HTTP request, he receives a JSON encoded error response such as the one below. This is an improvement over the default un-customisable raw text response.
